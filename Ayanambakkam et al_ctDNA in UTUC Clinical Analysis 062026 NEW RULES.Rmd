---
title: "Ayanambakkam et al_ctDNA in UTUC Clinical Analysis 062026 NEW RULES"
output:
  html_document:
    df_print: paged
---
...
rm(list=ls())
setwd("~/Downloads")
circ_data <- read.csv("RWE UTUC_Clinical Data_NEW RULES.csv")
...
library(swimplot)
setwd("~/Downloads") 
clinstage<- read.csv("RWE UTUC_OP 062026 MS NEW RULES.csv")
...
rm(list=ls())
setwd("~/Downloads")
dt <- read_xlsx("CLIA UTUC Time dependent data_NEWRULES.xlsx") |>
  clean_names() |>
  mutate(across(
...
rm(list=ls())
setwd("~/Downloads")
df <- read.csv("RWE UTUC_OP 062026 MS NEW RULES.csv", stringsAsFactors = FALSE)
...
rm(list=ls())
setwd("~/Downloads")
df <- read.csv("CLIA UTUC ctDNA MTM_NEW RULES.csv", stringsAsFactors = FALSE)
...
```{r}
rm(list=ls())
csv_path <- "~/Downloads/CLIA UTUC_Lead Time pts_062025 NEW RULES.csv"
...
```{r}
rm(list=ls())
csv_path <- "~/Downloads/CLIA UTUC ctDNA Velocity_NEW RULES.csv"  # <- adjust if needed
...
slope_threshold <- eligible %>%
  group_by(PatientName) %>%
  filter(n() >= 2, n_distinct(daysCR.months) >= 2) %>%
  summarise(slope = coef(lm(log10(MTM_mL) ~ daysCR.months))[["daysCR.months"]], .groups = "drop") %>%
  summarise(threshold = median(slope, na.rm = TRUE)) %>%
  pull(threshold)

cat(sprintf("Slow vs. fast growth threshold (median log10 MTM/mL per month slope): %.4f\n", slope_threshold))
```
